    ...
    <div class="overflow-x-auto">
      <table class="min-w-full table-fixed whitespace-nowrap text-sm md:text-base">
        ...
        <tbody>
          ${
              rowsHtml ||
              `<tr><td colspan="8" class="p-6 text-center text-gray-500">No matches.</td></tr>`
          }
        </tbody>
      </table>
    </div>`;
...
onAuthStateChanged(auth, async (user) => {
    ...
    const userTeamSuffixesByEvent = {};
    teamsSnap.forEach((teamDoc) => {
        const t = teamDoc.data();
        if (t.member_emails && t.member_emails.includes(user.email)) {
            const suffix = teamDoc.id.includes("__")
                ? teamDoc.id.split("__").pop()
                : teamDoc.id;
            if (!userTeamSuffixesByEvent[t.event_id])
                userTeamSuffixesByEvent[t.event_id] = new Set();
    ...
    const teamNameByEventAndSuffix = {};
    teamsSnap.forEach((teamDoc) => {
        const t = teamDoc.data();
        const suffix = teamDoc.id.includes("__")
            ? teamDoc.id.split("__").pop()
            : teamDoc.id;
        if (!teamNameByEventAndSuffix[t.event_id])
            teamNameByEventAndSuffix[t.event_id] = {};
    ...
    function renderEvent(eventId) {
        const eventMatches = allMatches
            ...
            .filter(
                (m) =>
                    !isPlaceholder(m.competitor_a?.id, m) &&
                    !isPlaceholder(m.competitor_b?.id, m)
            )
            .filter((m) => {
                ...
                const teamInvolved =
                    (aId && suffixSet.has(aId)) || (bId && suffixSet.has(bId));
                const individual =
                    aId === user.uid ||
                    bId === user.uid ||
                    m.competitor_a?.email === user.email ||
                    m.competitor_b?.email === user.email;
        ...
        const rows = eventMatches
            .map((m) => {
                const eventTeamNames = teamNameByEventAndSuffix[eventId] || {};
                const compA =
                    eventTeamNames[m.competitor_a?.id] ||
                    m.competitor_a?.name ||
                    m.competitor_a?.id ||
                    "-";
                const compB =
                    eventTeamNames[m.competitor_b?.id] ||
                    m.competitor_b?.name ||
                    m.competitor_b?.id ||
                    "-";
    ...
    let currentBadminton = badmintonSubtabs[0]?.dataset.sport || BADMINTON_EVENTS[0];
    ...
    const openTop = (btn) => {
        ...
        if (sport === "badminton") {
            ...
            const activeSub =
                badmintonSubtabs.find((x) => x.dataset.sport === currentBadminton) ||
                badmintonSubtabs[0];
    ...
    let firstEventWithData = null;
    for (const e of EVENTS) {
        const any = allMatches.some(
            (m) =>
                m.event_id === e &&
                (((userTeamSuffixesByEvent[e] &&
                    (userTeamSuffixesByEvent[e].has(m.competitor_a?.id) ||
                        userTeamSuffixesByEvent[e].has(m.competitor_b?.id))) ||
                    m.competitor_a?.id === user.uid ||
                    m.competitor_b?.id === user.uid ||
                    m.competitor_a?.email === user.email ||
                    m.competitor_b?.email === user.email))
        );
        if (any) {
            firstEventWithData = e;
            break;
        }
    }

    if (firstEventWithData && BADMINTON_EVENTS.includes(firstEventWithData)) {
        currentBadminton = firstEventWithData;
    ...
    if (firstEventWithData) {
        const top = topTabs.find((x) => x.dataset.sport === firstEventWithData);
        if (top) {
            openTop(top);
            return;
        }
    ...
    const fallbackTop = topTabs.find((x) => x.dataset.sport === "badminton") || topTabs[0];
    if (fallbackTop) openTop(fallbackTop);
});
